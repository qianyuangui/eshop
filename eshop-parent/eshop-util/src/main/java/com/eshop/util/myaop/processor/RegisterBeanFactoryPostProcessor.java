 ...
 *
 */
public class RegisterBeanFactoryPostProcessor implements
	...
	public void postProcessBeanFactory(
		...
		// 获取所有的bdName
		String[] beanDefinitionNames = configurableListableBeanFactory
		...
		for (String beanDefinitionName : beanDefinitionNames) {
			...
			if (beanDefinition instanceof AnnotatedBeanDefinition) {
				...
				for (String annotation : Annotations)
					if (annotation
							.equals(ConfigurationUtil.AOP_POINTCUT_ANNOTATION))
	...
	private void doScan(String className, Method method, Annotation annotation) {
		...
		proxyBeanHolder
				.setAnnotationName(annotation.annotationType().getName());
		// 获取注解上的所有方法
		Method[] annotationMethods = annotation.annotationType()
				.getDeclaredMethods();
		String packagePath = null;
		for (Method annotationMethod : annotationMethods) {
			if (annotationMethod.getName().equals("value")) {
				try {
					packagePath = (String) annotationMethod.invoke(annotation,
							null);
		...
		if (!packagePath.isEmpty()) {
			...
			for (File wjFile : fileList) {
				if (wjFile.isFile()) {// 判断是否为文件
					String targetClass = packagePath + "."
							+ wjFile.getName().replace(".class", "");
					try {
						lroxyBeanHolderList = ConfigurationUtil.classzzProxyBeanHolder
								.get(targetClass);
					} catch (Exception e) {
					}
					...
					ConfigurationUtil.classzzProxyBeanHolder.put(targetClass,
							lroxyBeanHolderList);
				}
			}
